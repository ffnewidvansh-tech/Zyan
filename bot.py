...
# ============================================================
# PROCESSING ANIMATION - BOXES FILLING WITH LIVE %
# ============================================================
def show_processing_animation(chat_id, action_text):
    """Boxes ek line me fill hote hue live percentage ke sath"""
    steps = [
        ("⬛⬜⬜⬜⬜⬜⬜⬜⬜⬜", "10%"),
        ("⬛⬛⬜⬜⬜⬜⬜⬜⬜⬜", "29%"),
        ("⬛⬛⬛⬛⬛⬜⬜⬜⬜⬜", "50%"),
    ...
    msg = bot.send_message(chat_id, f"⏳ {action_text}\n\n⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜ 0%")
    
    for boxes, percent in steps:
        time.sleep(0.4)  # Live animation feel
        try:
...
def confirm_ban_callback(call):
    try:
        ...
        # ===== ANIMATION SHOW (Banning) =====
        anim_msg = show_processing_animation(call.message.chat.id, "BANNING ACCOUNT...")
...
# ============================================================
# BAN CHECK - ANIMATION + FULL DATA + JSON FILE + JSON BLOCK
# ============================================================
...
def process_ban_check(message):
    ...
    # ===== 1️⃣ PROCESSING ANIMATION (boxes fill hote hue) =====
    anim_msg = show_processing_animation(message.chat.id, f"CHECKING UID: {uid_input}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Status ke hisaab se GREEN/RED
            if "not banned" in str(ban_status).lower():
                status_emoji = "🟢"
                status_word = "NOT BANNED ✅"
            elif "banned" in str(ban_status).lower():
                status_emoji = "🔴"
                status_word = "BANNED ❌"
            else:
                status_emoji = "🟡"
                status_word = str(ban_status).upper()

            # ===== 4️⃣ FULL INFO TEXT RESPONSE =====
            text = f"""
🟢 ═══《 🔍 BAN CHECK RESULT 》═══ 🔴
...
{status_emoji} ├─ Reason: {ban_reason}
{status_emoji} ├─ Ban Time: {ban_time}
{status_emoji} ├─ Duration: {ban_duration}
...
"""
            ...
            # ===== 5️⃣ JSON DATA FILE SEND =====
            with open(json_filename, "rb") as f:
                bot.send_document(
                    message.chat.id,
                    f,
                    caption=f"🟢 📄 JSON Data - UID: {uid_input}"
                )
            os.remove(json_filename)  # Send ke baad delete

            # ===== 6️⃣ JSON FORMATTED BLOCK (GREEN/RED STYLE) =====
            json_str = json.dumps(data, indent=2, ensure_ascii=False)
            if len(json_str) > 3800:
                json_str = json_str[:3800] + "\n...TRUNCATED..."
            json_str = _html.escape(json_str)

            json_block = f"""🟢🔴 ═══《 📄 JSON DATA 》═══ 🟢🔴
...
🟢🔴 ═══════════════════════"""
            try:
                bot.send_message(message.chat.id, json_block, parse_mode="HTML")
            except:
                _send_pe(message.chat.id, f"🟢 JSON DATA:\n<code>{json_str}</code>")
