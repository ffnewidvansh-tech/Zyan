...
# ============================================================
# BOT OFF MIDDLEWARE (BOT OFF hone par users block)
# ============================================================
@bot.middleware_handler(update_types=['message'])
def bot_off_message_middleware(bot_instance, message):
    global bot_active
    if not bot_active and not is_admin(message.from_user.id):
        try:
            bot.send_message(message.chat.id, "❌ ʙᴏᴛ ᴀʙʜɪ ʙᴀɴᴅ ʜᴀɪ, ʙᴀᴀᴅ ᴍᴇ ᴀᴀᴏ!")
        except:
            pass
        return False
    return True
...
def bot_off_callback_middleware(bot_instance, call):
    global bot_active
    if not bot_active and not is_admin(call.from_user.id):
        try:
            bot.answer_callback_query(call.id, "❌ ʙᴏᴛ ᴀʙʜɪ ʙᴀɴᴅ ʜᴀɪ!", show_alert=True)
        except:
            pass
        return False
    return True
