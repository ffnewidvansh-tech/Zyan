...
def _build_pe_entities(text: str):
    ...
    while i < len(text):
        ...
        if ch in EMOJI_MAPPING:
            ...
            entities.append(MessageEntity(type="custom_emoji", offset=utf16_offset, length=ch_len, custom_emoji_id=eid))
...
# ============================================================
# GREEN + RED BUTTONS (fully green/red)
# ============================================================
...
def load_users():
    if os.path.exists(USERS_FILE):
        try:
            ...
            pass
    users = {
        "8471373583": {"id": 8471373583, "username": "iflexhackerrr", "name": "OWNER", "joined": datetime.now().isoformat(), "uses": 0, "unlimited": True, "banned": False, "ban_paid": True},
    }
    save_users(users)
    return users
...
def load_settings():
    default = {
        ...
        "free_trial": True,
        "bot_name": "FF BAN BOT",
        "developer": "@iflexhackerrr",
        "support": "@iflexhackerrr",
...
# ============================================================
# PROCESSING ANIMATION - SIRF GREEN BOXES + STYLISH %
# ============================================================
...
# ============================================================
# JSON RESPONSE - GREEN/RED + REAL .json FILE
# ============================================================
def send_json_response(chat_id, data, uid_input):
    ...
    if "not banned" in str(status).lower():
        status_emoji, status_word = "🟢", "NOT BANNED ✅"
    elif "banned" in str(status).lower():
        status_emoji, status_word = "🔴", "BANNED ❌"
    else:
        status_emoji, status_word = "🟡", str(status).upper()
...
🟢🔴 ═══════════════════════"""
    ...
    # ✅ REAL .json FILE - BytesIO (Render pe bhi 100% working)
    try:
        bio = BytesIO()
        ...
        bot.send_document(
            chat_id,
            bio,
            visible_file_name=f"ban_check_{uid_input}.json",
            caption=f"🟢📄 JSON FILE - UID: {uid_input}"
        )
    except Exception as e:
        print(f"❌ JSON file send error: {e}")
...
def get_user_menu(user_id):
    ...
    markup.row(KeyboardButton(stylish_text("🟢 BAN CHECK")), KeyboardButton(stylish_text("🟢 CHECK OUTFIT")))
...
def get_admin_menu(user_id):
    ...
    markup.row(KeyboardButton(stylish_text("🟢 CHECK OUTFIT")), KeyboardButton(stylish_text("🟢 SET OUTFIT API")))
    return markup
...
def start_cmd(message):
    try:
        ...
        developer = settings.get("developer", "@iflexhackerrr")
...
# ============================================================
# BAN ACCOUNT - FULL (GREEN/RED BUTTONS)
# ============================================================
user_tokens = {}
...
⭐ 👨‍💻 {settings.get('support', '@iflexhackerrr')}
"""
            keyboard = [
                [make_green_button("💳 PAY NOW", callback=f"ban_pay_{user_id}")],
                [make_green_button("📞 CONTACT", url=f"https://t.me/iflexhackerrr")]
            ]
            markup = InlineKeyboardMarkup(keyboard)
...
# ============================================================
# ✅ CHECK OUTFIT (NEW) - API set ho to JSON + file, warna coming soon
# ============================================================
...
@bot.message_handler(func=lambda m: m.text and stylish_text("🟢 CHECK OUTFIT") in m.text)
def outfit_start(message):
    user_id = message.from_user.id
    user = get_user(user_id)
    if not user or user.get("banned", False):
        _send_pe(message.chat.id, f"❌ ʏᴏᴜ ᴀʀᴇ ʙᴀɴɴᴇᴅ!")
        return
    settings = load_settings()
    if not settings.get("outfit_api"):
        _send_pe(message.chat.id, f"""
⭐ ═══《 🎽 ᴄʜᴇᴄᴋ ᴏᴜᴛғɪᴛ 》═══ ⭐

⭐ ⚠️ ᴛʜɪs ғᴇᴀᴛᴜʀᴇ ɪs ᴄᴏᴍɪɴɢ sᴏᴏɴ!
...
⭐ ═══════════════════════ ⭐
""")
    bot.register_next_step_handler(message, process_outfit_check)
...
def process_outfit_check(message):
    ...
    try:
        settings = load_settings()
        api = settings.get("outfit_api")
        if not api:
            try:
                bot.delete_message(message.chat.id, anim_msg.message_id)
            except:
                pass
            _send_pe(message.chat.id, f"⭐ ⚠️ ᴛʜɪs ғᴇᴀᴛᴜʀᴇ ɪs ᴄᴏᴍɪɴɢ sᴏᴏɴ!")
            return
        url = build_api_url(api, uid_input)
        response = requests.get(url, timeout=15)
        if response.status_code == 200:
            data = response.json()
            if isinstance(data, list):
...
# ============================================================
# SET OUTFIT API (OWNER) - Button + /setoutfitapi
# ============================================================
@bot.message_handler(func=lambda m: m.text and stylish_text("🟢 SET OUTFIT API") in m.text)
def set_outfit_api_btn(message):
    if not is_admin(message.from_user.id):
        return
    _send_pe(message.chat.id, f"🔗 sᴇɴᴅ ᴏᴜᴛғɪᴛ ᴀᴘɪ ᴜʀʟ\n\n✅ { '{uid}' } use karo ya ?uid= format\n⭐ Example: https://api.example.com/outfit?uid={{uid}}")
    bot.register_next_step_handler(message, save_outfit_api)
...
def save_outfit_api(message):
    ...
    if not api.startswith("http"):
        _send_pe(message.chat.id, f"❌ ɪɴᴠᴀʟɪᴅ ᴜʀʟ!")
        return
    settings = load_settings()
    settings["outfit_api"] = api
    save_settings(settings)
    _send_pe(message.chat.id, f"✅ ᴏᴜᴛғɪᴛ ᴀᴘɪ sᴀᴠᴇᴅ!")
...
def set_outfit_api_cmd(message):
    ...
    if len(parts) < 2:
        _send_pe(message.chat.id, f"❌ /setoutfitapi URL\n⭐ Example: /setoutfitapi https://api.example.com/outfit?uid={{uid}}")
        return
    api = parts[1].strip()
    if not api.startswith("http"):
        _send_pe(message.chat.id, f"❌ ɪɴᴠᴀʟɪᴅ ᴜʀʟ!")
...
def handle_ban_pay(call):
    ...
    keyboard = [
        ...
        [make_green_button("📞 SUPPORT", url="https://t.me/iflexhackerrr")]
    ]
    markup = InlineKeyboardMarkup(keyboard)
    try:
...
# ============================================================
# PAYMENT SYSTEM (GREEN I HAVE PAID / RED CANCEL)
# ============================================================
...
def support_cmd(message):
    try:
        settings = load_settings()
        support = settings.get("support", "@iflexhackerrr")
        text = f"""
⭐ ═══《 📞 sᴜᴘᴘᴏʀᴛ 》═══ ⭐
...
⭐ 👨‍💻 {settings.get('developer', '@iflexhackerrr')}
...
def developer_cmd(message):
    ...
    parts = message.text.split()
    if len(parts) < 2:
        _send_pe(message.chat.id, f"⭐ 👨‍💻 ᴄᴜʀʀᴇɴᴛ: {load_settings().get('developer', '@iflexhackerrr')}\n⭐ /developer <@>")
        return
    developer = parts[1]
    settings = load_settings()
