...
# ============================================================
# STYLISH QR TEXT FUNCTION
# ============================================================
def get_stylish_qr_text(upi, price):
    """QR ke neeche stylish characters ke saath text"""
    stylish_emojis = ["⭐", "✨", "🔥", "💎", "👑", "💰", "💥", "🌟"]
...
def how_to_get_token_btn(message):
    try:
        how_to_get_token(message)
    except Exception as e:
...
def all_commands_btn(message):
    all_commands_cmd(message)
...
# ============================================================
# PAYMENT SYSTEM WITH STYLISH QR TEXT
# ============================================================
...
"""
            keyboard = [
                [make_blue_button("✅ ᴀᴘᴘʀᴏᴠᴇ", callback=f"admin_approve_{user_id}")],
...
"""
        ...
        for uid, data in list(users.items())[:10]:
            keyboard.append([
                make_blue_button(f"👤 {data.get('name', 'Unknown')}", callback=f"user_action_{uid}")
            ])
        keyboard.append([make_red_button("ᴄʟᴏsᴇ", callback="close_menu")])
        markup = InlineKeyboardMarkup(keyboard)
...
@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("user_action_"))
def user_action_callback(call):
    try:
        ...
        if not user:
            _send_pe(call.message.chat.id, f"❌ ᴜsᴇʀ ɴᴏᴛ ғᴏᴜɴᴅ!")
            bot.answer_callback_query(call.id)
            return
...
✅ 💎 {'ᴜɴʟɪᴍɪᴛᴇᴅ' if user.get('unlimited') else 'ғʀᴇᴇ'}
✅ 🚫 {'ʙᴀɴɴᴇᴅ' if user.get('banned') else 'ᴀᴄᴛɪᴠᴇ'}
✅ 👑 {'ᴀᴅᴍɪɴ' if user_id in ADMIN_IDS else 'ɴᴏᴛ ᴀᴅᴍɪɴ'}

✅ ═══════════════════════ ✅
"""
        
        keyboard = [
            [make_green_button("ᴀᴘᴘʀᴏᴠᴇ", callback=f"approve_{user_id}")],
            [make_red_button("ᴅɪsᴀᴘᴘʀᴏᴠᴇ", callback=f"disapprove_{user_id}")],
            [make_red_button("ʙᴀɴ", callback=f"ban_user_{user_id}")],
            [make_green_button("ᴜɴʙᴀɴ", callback=f"unban_user_{user_id}")],
            [make_blue_button("ᴀᴅᴅ ᴀᴅᴍɪɴ", callback=f"add_admin_{user_id}")],
            [make_red_button("ʀᴇᴍᴏᴠᴇ ᴀᴅᴍɪɴ", callback=f"remove_admin_{user_id}")],
            [make_red_button("ᴄʟᴏsᴇ", callback="close_menu")]
        ]
        markup = InlineKeyboardMarkup(keyboard)
...
@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("disapprove_"))
def disapprove_user_callback(call):
    try:
        if not is_admin(call.from_user.id):
            _send_pe(call.message.chat.id, f"❌ ᴜɴᴀᴜᴛʜᴏʀɪᴢᴇᴅ!")
            bot.answer_callback_query(call.id)
        ...
        user_id = int(call.data.split("_")[1])
        _send_pe(call.message.chat.id, f"❌ ᴜsᴇʀ {user_id} ᴅɪsᴀᴘᴘʀᴏᴠᴇᴅ!")
        bot.answer_callback_query(call.id)
    except Exception as e:
        print(f"❌ Disapprove callback error: {e}")

@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("ban_user_"))
def ban_user_callback(call):
    try:
        ...
        _send_pe(call.message.chat.id, f"✅ ᴜsᴇʀ {user_id} ʙᴀɴɴᴇᴅ!")
        try:
            bot.send_message(user_id, f"❌ ʏᴏᴜ ʜᴀᴠᴇ ʙᴇᴇɴ ʙᴀɴɴᴇᴅ!")
        except:
            pass
        bot.answer_callback_query(call.id)
    except Exception as e:
        print(f"❌ Ban callback error: {e}")

@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("unban_user_"))
def unban_user_callback(call):
    try:
        ...
        _send_pe(call.message.chat.id, f"✅ ᴜsᴇʀ {user_id} ᴜɴʙᴀɴɴᴇᴅ!")
        bot.answer_callback_query(call.id)
    except Exception as e:
        print(f"❌ Unban callback error: {e}")

@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("add_admin_"))
def add_admin_callback(call):
    try:
        ...
        if user_id not in ADMIN_IDS:
            ...
            _send_pe(call.message.chat.id, f"✅ ᴜsᴇʀ {user_id} ᴀᴅᴅᴇᴅ ᴀs ᴀᴅᴍɪɴ!")
        else:
            _send_pe(call.message.chat.id, f"⚠️ ᴀʟʀᴇᴀᴅʏ ᴀᴅᴍɪɴ!")
        bot.answer_callback_query(call.id)
    except Exception as e:
        print(f"❌ Add admin callback error: {e}")

@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("remove_admin_"))
def remove_admin_callback(call):
    try:
        ...
        if user_id in ADMIN_IDS and user_id != OWNER_ID:
            ...
            _send_pe(call.message.chat.id, f"✅ ᴀᴅᴍɪɴ ʀᴇᴍᴏᴠᴇᴅ ғᴏʀ {user_id}!")
        else:
            _send_pe(call.message.chat.id, f"❌ ᴄᴀɴɴᴏᴛ ʀᴇᴍᴏᴠᴇ ᴏᴡɴᴇʀ!")
        bot.answer_callback_query(call.id)
    except Exception as e:
        print(f"❌ Remove admin callback error: {e}")
