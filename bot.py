...
# ============================================================
# FULLY GREEN / RED BUTTONS (style=success/danger - koi dot nahi)
# ============================================================
def make_green_button(text, callback=None, url=None):
    label = stylish_text(text)
    try:
        if callback: return InlineKeyboardButton(text=label, style="success", callback_data=callback)
        if url:      return InlineKeyboardButton(text=label, style="success", url=url)
        return InlineKeyboardButton(text=label, style="success")
    except:
        if callback: return InlineKeyboardButton(text=label, callback_data=callback)
        if url:      return InlineKeyboardButton(text=label, url=url)
        return InlineKeyboardButton(text=label)

def make_red_button(text, callback=None, url=None):
    label = stylish_text(text)
    try:
        if callback: return InlineKeyboardButton(text=label, style="danger", callback_data=callback)
        if url:      return InlineKeyboardButton(text=label, style="danger", url=url)
        return InlineKeyboardButton(text=label, style="danger")
    except:
        if callback: return InlineKeyboardButton(text=label, callback_data=callback)
        if url:      return InlineKeyboardButton(text=label, url=url)
        return InlineKeyboardButton(text=label)
...
def admin_menu_markup():
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(
        make_green_button("BOT OFF", callback="adm_botoff"),
        make_green_button("BOT ON", callback="adm_boton"),
...
# ============================================================
# BAN ACCOUNT FLOW (GREEN YES / RED NO)
# ============================================================
user_tokens = {}
    ...
    try:
        ...
        if message.photo:
            ...
            kb = InlineKeyboardMarkup([
                ...
                [make_red_button("DISAPPROVE", callback=f"admin_disapprove_{user_id}")]
            ])
            admin_text = f"""
⭐ ═══《 💰 ɴᴇᴡ ᴘᴀʏᴍᴇɴᴛ 》═══ ⭐
...
def admin_callback(call):
    try:
        ...
        if action == "adm_botoff":
            global bot_active
            bot_active = False
            _send_pe(chat, "✅ 🔴 ʙᴏᴛ ɪs ɴᴏᴡ ᴏғғʟɪɴᴇ!")
        elif action == "adm_boton":
            global bot_active
            bot_active = True
            _send_pe(chat, "✅ 🟢 ʙᴏᴛ ɪs ɴᴏᴡ ᴏɴʟɪɴᴇ!")
